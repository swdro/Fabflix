...
public class UpdateSecurePassword {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // encrypt passwords in employee
        System.out.println("Encrypting passwords for employees table");

        // change the customers table password column from VARCHAR(20) to VARCHAR(128)
        String alterEmployeesQuery = "ALTER TABLE employees MODIFY COLUMN password VARCHAR(128)";
        int alterEmployeesResult = statement.executeUpdate(alterEmployeesQuery);
        System.out.println("altering customers table schema completed, " + alterEmployeesResult + " rows affected");

        // get the ID and password for each customer
        String employeesQuery = "SELECT email, password from employees";

        ResultSet EmployeesRS = statement.executeQuery(employeesQuery);
        ...
        while (EmployeesRS.next()) {
            ...
            // generate the update query
            String updateEmployeesQuery = String.format("UPDATE employees SET password='%s' WHERE email='%s';", encryptedPassword,
                    email);
            updateQueryList.add(updateEmployeesQuery);
        }
        EmployeesRS.close();
        ...
        int employeesUpdateCount = 0;
        for (String updateQuery : updateQueryList) {
            int updateResult = statement.executeUpdate(updateQuery);
            employeesUpdateCount += updateResult;
        }
        System.out.println("updating password completed, " + count + " rows affected");
